...
export interface PublikationCategory {
  h: string;
  meta: string;
  items: { y: string; t: string }[];
}

export interface NetzwerkItem {
  id: string;
  t: string;
  n: string;
}
...
export interface AKPData {
  ...
  filters: string[];
  bestandThemen: { n: string; t: string; d: string }[];
  caseAspekte: { n: string; t: string; d: string }[];
  kompetenzen: { n: string; t: string; d: string; tags: string[] }[];
  timeline: { y: string; l: string; active: boolean }[];
  stats: { v: string; l: string }[];
  international: string[];
  publikationen: PublikationCategory[];
  netzwerk: NetzwerkItem[];
  kontakt: {
    name: string;
    street: string;
    city: string;
...
export const AKP_DATA: AKPData = {
  ...
  projects: [
    ...
    { id: "klinik", title: "Privatklinik Hundekehlestraße", loc: "Berlin", typ: "Klinik / Gesundheit", tags: ["Pflege & Gesundheit", "Berlin"], img: null,
      ctx: "Privatklinik mit klarer Funktions­ordnung, Orientierung and kalibriertem Tageslicht in den Behandlungs­bereichen." },
